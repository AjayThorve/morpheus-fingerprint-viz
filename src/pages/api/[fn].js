...
function symmetricDataGrid(df) {
  ...
  [...df.get("userID").unique()].forEach((v) => {
    [...df.get("time").unique()].forEach((i) => {
      const maxEventCountIP = df
        .filter(df.get("time").eq(i))
        .groupBy({ by: "userID" })
        .count()
        .get("time")
        .max();
      const EventCount = df.filter(
        df.get("time").eq(i).logicalAnd(df.get("userID").eq(v))
      ).numRows;
  ...
  [...df.get("userID").unique()].forEach((v) => {
    paddingTime.forEach((i) => {
      ...
      d.row.push(...Array(nrows).fill(v));
      d.time.push(...Array(nrows).fill(i));
      randomAnomalyValues.push(
        ...Array(nrows).fill(anomalyValues[Math.floor(Math.random() * 5)])
      );
    });
  });
...
export default async function handler(req, res) {
  ...
  if (fn == "getUniqueIDs") {
    ...
    if (finalData.numRows > 0) {
      ...
      finalData = finalData.assign({
        x: finalData.get("x").add(offsetX),
        y: finalData.get("y").add(offsetY),
      });
    }
