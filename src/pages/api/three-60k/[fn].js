...
const maxCols = 40;
const names = Series.new(Array(60000/20).fill(['sgonzalez\n',
'ccameron\n',
'tanderson\n',
'jessicabrewer\n',
'abigailstanley\n',
...
function generateData(df, type="elevation"){
    ...
    if(type == "userIDs"){
        ...
        }).join({other: paddingDF, on:['userID'], how:'outer', lsuffix:'_r'}).select(['names']).head(2000);
    }
...
export default async function handler(req, res) {
    ...
    if(fn == "getUniqueIDs") {
        ...
        res.send(
            {
                totalEvents: Math.min(2000, events.numRows - events.get('anomalyScore').nullCount),
